...
def summarize_performance(step, g_model, x_test, y_test, n_samples=3):
    
    rand_nums = np.random.randint(0, x_test.shape[0], n_samples)   #[1,2,3]
    
    input_xtest = x_test[rand_nums]
    
    prediction = g_model.predict(input_xtest)
    prediction = ( prediction + 1 ) / 2.0
    
    input_xtest = ( input_xtest + 1 ) / 2.0
    
    ground_truth = y_test[rand_nums]
    ground_truth = ( ground_truth + 1 ) / 2.0
    # preds = []
    # for i in range(prediction.shape[0]):
    #     pred = ( prediction[i] + 1) / 2.0
    #     preds.append(pred)
    ...
    # pred1 = (prediction[0] + 1) / 2.0
    # pred2 = (prediction[1] + 1) / 2.0
    # pred3 = (prediction[2] + 1) / 2.0
...
def train(d_model, g_model, gan_model, dataset, x_test, y_test, n_epochs=100, n_batch=1):
	...
	for i in range(n_steps):
		...
		if (i+1) % (bat_per_epo * 1) == 0:
			summarize_performance(i, g_model, x_test, y_test)
